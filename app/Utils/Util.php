...
class Util
{
    ...
    public function ImageResizeAndUpload($image, $upload_directory, $width = null, $height = null)
    {
        $image_name = time() . '-' . uniqid() . '.' . $image->extension();
        $filePath = public_path('/' . $upload_directory);
        ...
        if (!empty($width) && !empty($height)) {
            ...
            $image->move(public_path() . '/' . $upload_directory . '/', $image_name);
        }
    ...
    /**
     * create sync data with pos
     *
     * @param int $length
     * @return string
     */
    function addSyncDataWithPos($model_name, $model_instance, $data, $request_type, $route_name)
    {
        if (env('ENABLE_POS_SYNC')) {
            $sync_data = new SyncDataWithPos();
            $sync_data->model_name = $model_name;
            $sync_data->model_id = $model_instance->id;
            $sync_data->pos_model_id = !empty($model_instance) ? $model_instance->pos_model_id : null;
            $sync_data->route_name = $route_name;
            $sync_data->request_type = $request_type;
            $sync_data->request_data = !empty($data) ? $data : [];
            $sync_data->is_synced = 0;
            $sync_data->save();
        }
    }
